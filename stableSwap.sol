...
interface IStableSwap {
  ...
  function mybalance (address _tokenaddress) external returns ( uint256 ) ;
	function external_stable_tokens ( address) external returns ( bool );
	function custom_stable_tokens ( address) external returns ( bool );
...
contract StableSwap is IStableSwap {
	...
	mapping ( address => bool ) public external_stable_tokens ;
	...
	function set_stable_token ( address _address , bool _status ) public onlyowner_or_admin ( msg.sender ) {
		require ( external_stable_tokens [ _address] != _status , "ERR() redundant call");
		external_stable_tokens [ _address] = _status ;
	}
	event Withdrawn (
	...
	function swap ( //
		...
		if (IStableSwap(address(this)).external_stable_tokens( _token_from) && 
				IStableSwap(address(this))._custom_stable_tokens( _token_to )) {
			_deposit ( msg.sender 
			 _token_from
			,  _token_to
			,  _amount_from
		...
		if (IStableSwap(address(this)).custom_stable_tokens( _token_from ) && 
				IStableSwap(address(this)).external_stable_tokens( _token_to )) {
